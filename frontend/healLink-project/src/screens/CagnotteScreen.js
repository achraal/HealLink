...
export default function CagnotteScreen() {
  ...
  const fetchCagnottes = async () => {
    try {
      const response = await fetch("http://" + ip + ":8000/cagnottes");
      ...
      <Modal visible={qrVisible} transparent animationType="fade">
        <View style={styles.qrOverlay}>
          <View style={styles.qrBox}>
            <Text style={styles.qrTitle}>📤 Partager cette cagnotte</Text>

            {selectedCagnotte && (
              <>
                <QRCode
                  value={"CAMP:" + selectedCagnotte.id}
                  size={220}
                />

                <Text style={styles.qrText}>
                  Scannez pour ouvrir cette cagnotte
                </Text>

                <Text style={styles.qrId}>
                  {"CAMP:" + selectedCagnotte.id}
                </Text>
              </>
            )}
